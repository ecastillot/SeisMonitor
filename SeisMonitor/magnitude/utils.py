...
def get_Ml(ampl,epi_dist,who="RSNC"):
    if who not in list(Ml_params.keys()):
        raise Exception(f"{who} not in Ml_params")

    ml_params = Ml_params[who]
    k = ml_params["a"]*math.log10(ml_params["r_ref"]/100) +\
        ml_params["b"]* (ml_params["r_ref"]-100) +3
    return math.log10(ampl * 1000) + ml_params["a"] * math.log10(epi_dist/ml_params["r_ref"]) +\
