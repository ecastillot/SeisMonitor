...
def write_stream(one_st,ppc_restrictions,mseed_storage,
	...
	if threshold != None:
		...
		if length < threshold:
			...
			printlog("info",'Downloaded: False',
	...
	if to_pick != None:
		...
		if mybatch < batch_size:
			comment = (f"This mseed only can be splitted in {int(mybatch)} batch size. "+\
					f"According to 'to_pick':{to_pick}' parameter, your "+\
					f"batch_size:{batch_size} is bigger. Then no download this mseed")
			# logger =logging.getLogger('Downloaded: False') 
			# logger.info(f'{mseed_filename}  {comment}')
			printlog("info",'Downloaded: False',
					f'{mseed_filename}  {comment}')
	...
	if os.path.isfile(mseed_filename) == False:
		...
		if download == False:
			...
			printlog("info",'Downloaded: True',
					f'{mseed_filename}  {comment}')

	else:
		# logger =logging.getLogger('Downloaded: Exist')
		# logger.info(f'{mseed_filename}  {comment}') 
		printlog("info",'Downloaded: Exist',
					f'{mseed_filename}  {comment}')
...
def get_st_according2preference(st,location_list,channel_list):
	...
	printlog("info",f"{stats.network}-{stats.station}: ",
