...
class MseedDownloader(object):
  def __init__(self,providers,restrictions,ppc_restrictions=None):
    """
    ...
    self.ppc_restrictions = ppc_restrictions

  def make_json(self, json_path,from_xml=None):
        """
        Parameters:
        -----------
        from_xml: str
        ...
        json_list = ut.makeJSON(json_path=json_path, 
  ...
  def download(self,mseed_storage, n_processor=None,
    ...
    """
    restrictions = self.restrictions
    ppc_restrictions = self.ppc_restrictions
    ...
    for chunkt,(starttime, endtime) in enumerate(times):
      ...
      try:
        ...
        if client.__module__ in ("obspy.clients.filesystem.sds"):
          st = ut.get_all_sdswaveforms(client=client,network=dld_restrictions.network,
