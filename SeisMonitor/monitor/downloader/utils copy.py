...
def get_merged_inv_and_json(providers):
	...
	for provider in providers:
		...
		av_nets = list(set([net.code for net in inv.networks]))
		av_stas = list(set([sta.code for net in inv.networks for sta in net.stations ]))
		av_nets.sort()
		av_stas.sort()
		av_nets = ",".join(av_nets)
		av_stas = ",".join(av_stas)

		provider.download_restrictions.network = av_nets
		provider.download_restrictions.station = av_stas
